...
public abstract class AgreementModel extends AbstractModel {
    ...
    public Boolean evaluateGT(String gtName, List<Property> properties) {
        GuaranteeTerm gt = getGuaranteeTerm(gtName);
        if (gt != null) {
            //Es necesario clonar
            Expression expr = gt.clone().getSlo().getExpression();
            for (Var v : Expression.getVars(expr)) {
                Property p = getProperty(v.getId().toString());
                if (p != null) {
                    if (Util.containsIdProperty(properties, p)) {
                        v.setValue(Util.getSameIdProperty(properties, p));
                    } else {
                        throw new NullPointerException("Property " + p
                                + " not found. " + "Are you sure you have declared it?");
                    }
                }
            }
    ...
    public Boolean evaluateGTByValues(String gtName, List<Entry<String, Number>> properties) {
        GuaranteeTerm gt = getGuaranteeTerm(gtName);
        if (gt != null) {
            //Es necesario clonar
            Expression expr = gt.clone().getSlo().getExpression();
            for (Var v : Expression.getVars(expr)) {
                Property p = getProperty(v.getId().toString());
                if (p != null) {
                    if (Util.containsIdPropertyByValues(properties, p)) {
                        v.setValue(Util.getSameIdPropertyByValues(properties, p));
                    } else {
                        throw new NullPointerException("Property " + p
                                + " not found. " + "Are you sure you have declared it?");
                    }
