...
public class WhyAreNotCompliant extends CoreOperation {
	...
	public void analyze(AgreementModel template, AgreementModel offer) {
		...
		if ((Boolean) res.get("compliant")) {
			...
			if (((AgreementTemplate) template).getCreationConstraints().size() > 0) {
				...
				OperationResponse res2 = reasoner.whyNotImplies(csp_offer,
						csp_template_ccs);
		...
		if (!(Boolean) res.get("compliant")) {
			List<CSPConstraint> conflictingConstraints = putExplanation(res,
					csp_offer.clone(), csp_template.clone());
			classifyConflicts(conflictingConstraints,
					(AgreementTemplate) template, csp_template.clone(),
					translator);
		}
		// result.putAll(res);
	}
	...
	private List<CSPConstraint> putExplanation(OperationResponse res,
		...
		// detected
		// to be developed
		List<CSPConstraint> sourceTemplateConstraints = csp_template
				.getConstraints();
		...
		while (constraintsEmpty) {
			...
			while (itConflictResult.hasNext()) {
				...
				Iterator<CSPConstraint> itSourceConstraints = sourceConstraints
						.iterator();
				while (itSourceConstraints.hasNext()) {
					CSPConstraint cons = (CSPConstraint) itSourceConstraints
							.next();
					...
					if (conf.startsWith("ASSIG")
							&& conf.equalsIgnoreCase(constraint_name)) {
						if (confs == null || confs != null
								&& !confs.contains(cons.toString())) {
							constraints.add(cons);
							result.put("conflicts", cons.toString());
							conflictsForTemplate.remove(cons.getId());
		...
		Iterator<String> itConflictsForTemplate = conflictsForTemplate
				.iterator();
		while (itConflictsForTemplate.hasNext()) {
			String affected = (String) itConflictsForTemplate.next();

			Iterator<CSPConstraint> itSourceTemplateConstraints = sourceTemplateConstraints
					.iterator();
			while (itSourceTemplateConstraints.hasNext()) {
				CSPConstraint c = (CSPConstraint) itSourceTemplateConstraints
						.next();
				...
				if (affected.startsWith("ASSIG")
						&& affected.equalsIgnoreCase(constraint_name)) {
					if (affected == null || affected != null
							&& !affected.contains(c.toString())) {
						affectedTemplateConstraints.add(c);
						result.put("affectedTerms", c.toString());
					}
	...
	private void classifyConflicts(List<CSPConstraint> constraints,
			AgreementTemplate template, CSPModel csp_template,
		...
		if (moreRestrictive) {
			if (result.get("conflicts") == null)
				conflictType = "the offer has more restrictive terms";
			else
				conflictType = "more restrictive offer term";
		} else {
			CSPModel modelForClassify = confModel.clone().add(
					csp_template.negate()); // V,D,Conflict y noT
