...
public class MiAgreeVisitor implements iAgreeVisitor<Object> {
    ...
    @Override
    public Object visitGlobal_MonitorableProperties(iAgreeParser.Global_MonitorablePropertiesContext ctx) {

        for (iAgreeParser.PropertyContext prop : ctx.property()) {
            Property p = this.visitProperty(prop);
            if (p != null) {
                MonitorableProperty mp = new MonitorableProperty(p.getId(), p.getMetric());
                mp.setDefinitionReference(p.getDefinitionReference());
                mp.setExpression(p.getExpression());
                mp.setScope(Scope.Global);
                this.model.getAgreementTerms().getMonitorableProperties().put(mp.getId(), mp);
            }
        }
