...
public class TestExistDeadTerms implements TestOperation {
    ...
    public int[] execute(AgreementModel model, File currentFile,
        ...
        if (dt_op.getResult().getExistDeadTerms() != null) {
            deadterms = dt_op.getResult().getExistDeadTerms();
            if (deadterms) {
                result = (List<String>) dt_op.getResult().getConflicts();
            } else {
                result.add("NULL");
            }
            ...
            for (String res : result) {
                if (res.equals(expectedResult)) {
                    sb.append(Status.PASSED).append(" ").append(currentFile.getPath()).append(" (Result: ").append(result).append(", Expected: ").append(expectedResult).append(")");
                    results[0] = 1;
                    passed = true;
                }
            }
            ...
            if (!passed) {
                sb.append(Status.NOT_PASSED).append(" ").append(currentFile.getPath()).append(" (Result: ").append(result).append(", Expected: ").append(expectedResult).append(")");
                results[1] = 1;
            }
