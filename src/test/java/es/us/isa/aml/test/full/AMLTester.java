...
import es.us.isa.aml.test.full.AMLTesterConfig.DirOp;
import es.us.isa.aml.test.full.operations.TestAreCompliant;
...
public class AMLTester {
    ...
    public static String DIR = "full-pack/release-packs/pack-v0.1/wsag";
    ...
    public static String LOGFILE = "src/test/QA/testresult_";
    ...
    public static void main(String[] args) {
        ...
        //tester.convertFiles(currentDir, AgreementLanguage.WSAG, AgreementLanguage.IAGREE);
    ...
    public void analyzeFiles(File dir) {
        LOG.log(Level.INFO, "ANALYZING FILES...");
        long startTime = System.nanoTime();
        String timeStamp = new SimpleDateFormat(LOGFILE_DATE_FORMAT)
                .format(Calendar.getInstance().getTime());
        File log_file = new File(LOGFILE + timeStamp + ".txt");
        File[] metrics = null;
        ...
        try {
            ...
            for (Status status : Status.values()) {
                ...
                sb.append(status).append(" (").append(dir_total).append(")").append("\n");
                for (String currDir : dirs.keySet()) {
                    sb.append("\t").append(currDir).append(": ").append(dirs.get(currDir))
                            .append("\n");
                }
                sb.append("\n");
                total += dir_total;
            }
            LOG.log(Level.INFO, "ANALYSIS FINISHED ({0}ms). {1} files analyzed.", new Object[]{duration, total});
            LOG.log(Level.INFO, sb.toString());
            LOG.log(Level.INFO, "See log file: {0} for more details.", log_file.getPath());
                ...
                if (currentFile.isFile()) {
                    if (!analyzed_files.contains(currentFile.getName())) {
                        ...
                        if (TestUtil.isValidExtension(currentFile, CONFIG)) {
                            ...
                            if (model != null) {
                                ...
                                if (test_op != null) {
                                    ...
                                    if (test_op instanceof TestAreCompliant) {
                                        ...
                                        for (File file : files) {
                                            if (TestUtil.isValidExtension(file,
                                                ...
                                                } else if (model2.getDocType() == DocType.TEMPLATE) {
                                                    analyzed_files.add(file
                                                            .getName());
                                                    break;
                                                }
                                    ...
                                    StringBuilder sb = new StringBuilder(
                                            timeStamp + " ");
                                    sb.append(Status.NOT_SUPPORTED).append(" ").append(currentFile.getPath());
                        ...
                        String timeStamp = new SimpleDateFormat("("
                                ...
                                .getInstance().getTime());
                        StringBuilder sb = new StringBuilder(timeStamp + " ");
                        sb.append("ALREADY ANALYZED" + " ").append(currentFile.getPath());
                        log.append(sb.toString()).append("\n");
                    }
                } else {
                    prop = null;
    ...
    public void convertFiles(File dir, String destId, AgreementLanguage dst_lang) {
        ...
        if (!currentDir.mkdir()) {
            LOG.log(Level.INFO, "Directory {0} already exists. All data on the destination will be overwritten.", currentDir.getPath());
        }
        File[] metrics = null;
        for (File f : dir.listFiles()) {
            if (f.isDirectory()
                ...
                int size = f.listFiles().length;
                metrics = new File[size];
                System.arraycopy(f.listFiles(), 0, metrics, 0, size);
            }
        }
        ...
        long endTime = System.nanoTime();
        long duration = (endTime - startTime) / 1000000;
        LOG.log(Level.INFO, "CONVERSION FINISHED ({0}ms)", duration);
        LOG.log(Level.INFO, "{0} files was converted.", converted_files);
    }
                ...
                if (file.isFile()) {
                    ...
                    if (TestUtil.isValidExtension(file, CONFIG)) {
                        ...
                        if (model != null) {
                            ...
                            LOG.log(Level.SEVERE, "ERROR: {0}", file.getAbsolutePath());
                        }
                    } else {
                        File newFile = new File(currentDir + File.separator
                                + file.getName());
